jQuery(document).ready(function($){
    ...
    wacCartDeleteEvent = function() {
        $('.remove').on( 'click', function(){
            wacFixEventLose();
        });
    };

    wacFixEventLose = function() {
        // fix event lose when set qty to zero or item removed from the cart
        setTimeout(function(){ wacListenChange(); }, 500);
        setTimeout(function(){ wacListenChange(); }, 1500);
        setTimeout(function(){ wacListenChange(); }, 3000);
        setTimeout(function(){ wacListenChange(); }, 6000);
    };
